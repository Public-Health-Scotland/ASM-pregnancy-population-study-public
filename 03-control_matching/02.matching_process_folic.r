##02.matching_process_folic.r
# matching process script for folic acid stat analyses
...
df_min <- df %>% select(pregnancy_id, mother_upi, exposed_any_asm, CC_exposed_any_asm, gest_first_exposed,
                        year_conception,high_dose_folic_acid, gest_end_pregnancy)
...
match_condition <- function(x, outcome){
  ...
  matchit(exposed_any_asm ~ year_conception  ,
          data = x,
          exact = ~ year_conception + high_dose_folic_acid,
          antiexact = ~mother_upi,
...
x <- df_list_nomissing[[1]]
match1 <-matchit(exposed_any_asm ~ year_conception  ,
        data = x,
        exact = ~ year_conception + high_dose_folic_acid,
        antiexact = ~mother_upi,
        ratio = 10, replace = TRUE, verbose=TRUE)
