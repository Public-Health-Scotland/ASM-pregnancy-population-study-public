...
#### 12. healthboard at review###
hb_review <- aggregate_data(data, hb_27m, "Healthboard at review") 
...
compare_df_cols(totals, epilepsy, mental_health, pain, any_indication, conception_year,
                age, simd, baby_sex, bmi, smoke, folic, drug_alcohol, hb_review,parity,
                slc, gross, fm, prob, persoc, eb, any)
...
t1e <- bind_rows(total_incl_unkn, total_unkn, totals, epilepsy, mental_health, pain, any_indication,
                 conception_year, age, simd, baby_sex, bmi, smoke, folic, drug_alcohol,hb_review,parity,comorbs,
                 slc, gross, fm, prob, persoc, eb, any, total_missing, total_incomplete, total_invalid_date) %>%
...
compare_df_cols(totals, epilepsy, mental_health, pain, any_indication, conception_year, age, simd, baby_sex,
                bmi, smoke, folic, drug_alcohol,comorbs,
                slc, gross, fm, prob, persoc, eb, any)

conception_year <- conception_year %>% mutate(sub_indicator = as.character(sub_indicator))
drug_alcohol <- drug_alcohol %>% 
  mutate(sub_indicator = case_when(sub_indicator == 'no' ~ 0,
                                   sub_indicator == 'yes' ~ 1)) %>% 
  mutate(sub_indicator = as.character(sub_indicator))

t1e_matched <- bind_rows(totals, epilepsy, mental_health, pain, any_indication, conception_year, age, 
                         simd, baby_sex, bmi, smoke, folic, drug_alcohol,hb_review,parity,comorbs,
                         slc, gross, fm, prob, persoc, eb, any) %>%
  select(indicator, sub_indicator, n_exposed, percent_exposed, n_unexposed, percent_unexposed) %>% 
...
write_csv(final_t1e, paste0(folder_data_path,"Descriptives/table_1e.csv"))
