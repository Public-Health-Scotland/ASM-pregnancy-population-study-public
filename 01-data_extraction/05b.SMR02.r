...
smr02 <- data_smr02  %>% 
  mutate(mh_indication = case_when(substr(main_condition,1,3) %in% ment_health ~1, 
         ...
         epilepsy_indication = case_when(substr(main_condition,1,3) %in% epilepsy ~1, 
                                         ...
                                         substr(indication_for_operative_del,1,3) %in% epilepsy ~1, 
                                         T~0), 
  ...
  mutate(alcohol_drug_use = case_when(substr(main_condition,1,3) %in% alcohol_drug ~1, 
                                      ...
                                      drug_misuse==1 ~1, 
                                      injected_illicit_drugs %in% c(1,2,3) ~1,
                                      T~0)) %>% 
  filter(mh_indication==1 | epilepsy_indication==1 |migraine_pain_indication==1 |alcohol_drug_use ==1 )

smr02 <- smr02 %>% select(upi_number, dob, dr_postcode, admission_date, discharge_date,
                          epilepsy_indication, mh_indication, migraine_pain_indication, 
                          alcohol_drug_use )

saveRDS(smr02, paste0(data_path, "smra_extracts/smr02_indicators.rds"))
